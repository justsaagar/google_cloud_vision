import 'dart:io';

import 'package:flutter/scheduler.dart';
import 'package:google_cloud_vision/app/helper/app_enum.dart';
import 'package:google_cloud_vision/app/helper/extension_helper.dart';
import 'package:google_cloud_vision/app/utills/app_function.dart';
import 'package:google_cloud_vision/screen/home/landmark_detection/landmark_detection_screen.dart';
import 'package:google_cloud_vision/serialized/landmark_detection_result_model.dart';
import 'package:google_cloud_vision/serialized/message_model.dart';

class LandmarkDetectionHelper {
  final LandmarkDetectionScreenState state;
  File? image;
  List<MessageModel<LandmarkDetectionResult>> messages = [];
  bool isDetecting = false;

  LandmarkDetectionHelper(this.state) {
    SchedulerBinding.instance.addPostFrameCallback(
      (timeStamp) async {
        messages.add(MessageModel(
          isUser: false,
          messageType: MessageType.text,
          content: 'Hello! Please select an image to detect landmarks.',
        ));
        updateState();
      },
    );
  }

  void updateState() => state.controller?.update();

  Future<void> pickImage() async {
    File? pickedFile = await AppFunction.selectImageFromCameraOrGallery();
    if (pickedFile != null) {
      image = File(pickedFile.path);
    }
    updateState();
  }

  void clearImage() {
    image = null;
    updateState();
  }

  Future<void> detectLandmarks() async {
    if (image == null) return;

    try {
      messages.add(MessageModel(
        isUser: true,
        messageType: MessageType.image,
        content: image!.path,
      ));
      isDetecting = true;
      messages.add(MessageModel(
        isUser: false,
        messageType: MessageType.loading,
        content: '',
      ));
      updateState();

      final bytes = await image!.readAsBytes();
      clearImage();

      final landmarkDetectionResults = await state.controller?.landmarkDetectionRepository.detectLandmarks(bytes);

      messages.removeWhere((message) => message.messageType == MessageType.loading);
      isDetecting = false;

      messages.add(MessageModel(
        isUser: false,
        messageType: MessageType.detection,
        detectionType: DetectionType.landmark,
        content: '',
        detectionResults: landmarkDetectionResults!,
      ));
      updateState();
    } catch (e) {
      '$e'.logs();
      messages.removeWhere((message) => message.messageType == MessageType.loading);
      isDetecting = false;
      messages.add(MessageModel(
        isUser: false,
        messageType: MessageType.text,
        content: 'Error detecting landmarks: $e',
      ));
      updateState();
    }
  }
}
