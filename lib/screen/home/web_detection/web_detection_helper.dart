...
class WebDetectionHelper {
  final WebDetectionScreenState state;
  File? image;
  List<MessageModel<WebDetectionResult>> messages = [];
  bool isDetecting = false;

  WebDetectionHelper(this.state) {
    SchedulerBinding.instance.addPostFrameCallback(
      (timeStamp) async {
        messages.add(MessageModel(
          isUser: false,
          messageType: MessageType.text,
          content: 'Hello! Please select an image for web detection.',
        ));
        updateState();
      },
    );
  ...
  Future<void> detectWeb() async {
    ...
    try {
      ...
      final webDetectionResults = await state.controller?.webDetectionRepository.detectWeb(bytes);
      ...
      messages.add(MessageModel(
        ...
        detectionType: DetectionType.web,
        content: '',
        detectionResults: webDetectionResults!,
      ));
      updateState();
    } catch (e) {
      '$e'.logs();
      ...
      messages.add(MessageModel(
        isUser: false,
        messageType: MessageType.text,
        content: 'Error performing web detection: $e',
      ));
      updateState();
    }
  }
